...
/**
  ...
  * @retval 无
  */
static void GTP_Touch_Up(void)   //触屏输出无卡顿
{
  /*触笔释放，把pre xy 重置为负*/
...
//  */
//uint8_t TouchDev_GetPoint(POINT *pt)
//{
//  static int ts_state=TS_ACT_NONE;
//	int ret = 0;

//	ret = GTP_Execu(&pt->x,&pt->y);
//	
//  /* 通过GTP_Execu获取触摸坐标和状态 */
//	if(ret)
//	{
//			ts_state =TS_ACT_DOWN;
//			printf("state: %d, input_x: %d, input_y: %d\r\n", ts_state, pt->x,pt->y);
//	}
//	else
//	{
...
////	State.Layer = 0;
////	GUI_PID_StoreState(&State);
//	printf("xxxxxxxxxx: %d, yyyyyyyyyyyyyy: %d\r\n", pre_x, pre_y);
}
///**
