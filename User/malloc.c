...
__align(32) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0XD0000000)));					//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)
__align(32) uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((at(0x24000000)));					//内部DTCM内存池
//内存管理表
uint32_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];													//内部SRAM内存池MAP
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0XD0000000+MEM2_MAX_SIZE)));	//外部SRAM内存池MAP
uint32_t mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((at(0x24000000+MEM3_MAX_SIZE)));	//内部DTCM内存池MAP
//内存管理参数	   
